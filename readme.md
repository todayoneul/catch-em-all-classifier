...
- [연구 및 실험 방법론 (Methodology)](#연구-및-실험-방법론-methodology)
  ...
  - [5. ConvNeXt 미세 조정 (Modernized CNN)](#5-convnext-미세-조정-modernized-cnn)
  - [6. Swin Transformer 미세 조정 (Hierarchical Vision Transformer)](#6-swin-transformer-미세-조정-hierarchical-vision-transformer)
...
모델의 성능, 학습 속도(Epoch 당 소요 시간), 그리고 GPU 메모리 효율성(VRAM 사용량) 간의 트레이드오프(Trade-off)를 면밀히 평가하기 위해 **총 여섯 가지의 독립적인 실험 파이프라인**을 설계했습니다.
...
### 5. ConvNeXt 미세 조정 (Modernized CNN)
전통적인 CNN의 한계를 넘어 트랜스포머의 아키텍처적 장점을 역으로 흡수한 모던 CNN 모델인 **ConvNeXt-Tiny**(`facebook/convnext-tiny-224`)를 평가 파이프라인에 추가했습니다.
- 패치화(Patchify), 깊이별 합성곱(Depthwise Convolution), LayerNorm, GELU 활성화 함수 등 ViT의 설계 철학을 반영한 이 아키텍처가 CNN의 귀납적 편향을 유지한 채로 어떻게 ViT급의 성능을 달성하는지 검증합니다.

### 6. Swin Transformer 미세 조정 (Hierarchical Vision Transformer)
기존 ViT가 이미지를 고정된 크기의 패치로 한 번에 처리하여 지역적 디테일(Local Details) 파악에 취약했던 점을 보완하기 위해, 계층적 구조를 가진 **Swin-Tiny**(`microsoft/swin-tiny-patch4-window7-224`)를 도입했습니다.
- 작은 윈도우 단위에서 시작하여 계층적으로 영역을 병합(Patch Merging)해 나가는 구조적 특징이 진화 전후의 포켓몬처럼 미세한 외형적 차이를 가진 클래스들을 구별하는 데 어떤 이점을 제공하는지 분석합니다.
